...
// CreateVolume create a new volume from the given request. The function
// is idempotent
func (cs *controllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	// Keep a record of the requst access type
	...
	// fields in VolumeCapabilities are sane, The check above
	// is just enough to pass the "[Testpattern: Dynamic PV
	// (block volmode)] volumeMode should fail in binding
	// dynamic provisioned PV to PVC" storage E2E test.
	...
	if capacity >= maxStorageCapacity {
		return nil, status.Errorf(codes.OutOfRange, "Requested capability %d exceeds maximum allowed %d", capacity, maxStorageCapacity)
	}

	// Need to check for already existing volume name, and if found
	// check for the reqested capacity and already allocated capacity
	if exVol, err := getVolumeByName(req.GetName()); err == nil {
		// Since err is nil, it means the volume with the same name already exist
		// need to check if the size of the existing volume is same as in new
		// request
		if exVol.VolSize >= int64(req.GetCapacityRange().GetLimitBytes()) {
...
func (cs *controllerServer) ValidateVolumeCapabilities(ctx context.Context, req *csi.ValidateVolumeCapabilitiesRequest) (*csi.ValidateVolumeCapabilitiesResponse, error) {
	...
	for _, cap := range req.GetVolumeCapabilities() {
		...
		// A real driver would check the capabilities of the given volume
		// with the set of requested capabiltiies.
	}
...
// ControllerPublishVolume attaches the given volume to node
...
func (cs *controllerServer) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	if exSnap, err := getSnapshotByName(req.GetName()); err == nil {
		// Since err is nil, it means the snapshot with the same name already exist.
		// Need to check if the sourceVolumeId of existing snapshot is the same as in
		// new request.
	...
	if hostPathVolume.VolAccessType == blockAccess {
		...
		glog.V(4).Infof("Creating snapshot for Filesystem Mode Volume")
...
func (cs *controllerServer) ListSnapshots(ctx context.Context, req *csi.ListSnapshotsRequest) (*csi.ListSnapshotsResponse, error) {
	...
	// case 2: SourceVolumeId is not empty, return snapshot that match the source volume id.
	...
	// case3: no parameter is set, so we return all the snapshots.
	...
	var (
		...
		j   = startingToken
		entries = make(
			[]*csi.ListSnapshotsResponse_Entry, maxEntries,
		)
